...
app = FastAPI(
    title="CRTI Mock API",
    description="Hackathon-ready mock service for Compass demo data.",
    version="0.1.0",
)
...
PROCESSING = {
    ...
    "steps": [
        {
            "label": "Raw institutional data received",
            "value": "Selected contribution fields accepted under campaign policy",
        },
        {
            "label": "Processed inside confidential environment",
            "value": "TEE-enabled execution boundary",
        },
        {
            "label": "Persistent storage disabled",
            "value": "No raw payload persistence outside runtime",
        },
        {
            "label": "Deterministic engine executed",
            "value": "Benchmark, reliability, and dispersion metrics computed",
        },
        {
            "label": "Local explainer enabled",
            "value": "Summary layer restates computed outputs only",
        },
        {
            "label": "Derived outputs generated",
            "value": "Benchmark package and institution-scoped comparison",
        },
...
def root():
    return {
        "service": "CRTI Mock API",
        "status": "ok",
        "message": "Compass demo data service is running.",
    }
...
@app.get("/explainable-summary")
def get_explainable_summary():
    return EXPLAINABLE_SUMMARY
